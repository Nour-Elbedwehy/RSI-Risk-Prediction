...
Note: the pipeline was trained with a derived "RiskScore" feature:
    RiskScore = DailySittingHours + PainLevel - BreakCountPerDay
This is NOT collected from the user directly — it is computed here before
calling the model, since it is a workload signal derived from other inputs
...
MODEL_PATH = os.path.join(APP_DIR, "rsi_risk_model.joblib")
...
feature_names = bundle["feature_names"]  # includes the derived RiskScore column

# Fields collected directly from the user (RiskScore is computed, not asked)
RAW_NUMERIC_FIELDS = [
...
# Allowed categorical values (must match what the pipeline's OneHotEncoder saw in training)
CATEGORY_OPTIONS = {
    "Gender": ["Female", "Male"],
    "Posture": ["Good", "Poor"],
...
# Sensible input ranges taken from the training dataset's describe() stats
NUMERIC_RANGES = {
...
def predict():
    ...
    # ---- derive RiskScore exactly as done during training ----
